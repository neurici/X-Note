$(document).ready(function() {
	...
	$(".logout-button").on("click", function() {
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					close_note();
					notify("Încărcare...", "Ai fost deconectat.", "blue", 4000);
					setTimeout(function() {
						location.reload();
					}, 2500);
				}
				else {
					notify("Eroare...", "Ceva nu a funcționat corect.", "red", 4000);
				}
			}
		});
	});
	...
	$(".delete-button").on("click", function() {
		...
		open_user_confirmation("Ești sigur?", "Nota va fi ștearsă definitiv.", file, "delete-note");
	});
	...
	$(".link-button").on("click", function() {
		notify("Copiere...", "Un link public către nota dvs. a fost copiat în clipboard..", "blue", 4000);
	...
	$(".lock-button").on("click", function() {
		...
		open_note_lock("unlocked", "lock-note", "Setare Parolă Blocare", "Modificările vor fi salvate automat.");
	});
	$(".unlock-button").on("click", function() {
		$(".note-lock-wrapper .close-icon").attr("id", "change");
		open_note_lock("locked", "unlock-note", "Eliminare Parolă Blocare", "Modificările vor fi salvate automat.");
	});
	$(".relock-button").on("click", function() {
		$(".note-lock-wrapper .close-icon").attr("id", "change");
		open_note_lock("locked", "relock-note", "Schimbare Parolă Blocare", "Modificările vor fi salvate automat.");
	});
	$(".rename-button").on("click", function() {
		...
		$(".note-title-edit-text").html("Care este noul titlu al notei?");
	...
	$(".copy-button").on("click", function() {
		notify("Copiere...", "Conținutul acestei note a fost copiat în clipboard", "blue", 4000);
	...
	$(".compose-button").on("click", function() {
		...
		$(".note-title-edit-text").html("Care este titlu notei??");
		$(".note-title-edit-input").focus().attr("placeholder", "Titlu...");
	});
	$(".editor-wrapper").delegate(".editor-empty", "click", function() {
		$(".note-title-edit-wrapper").show();
		$(".note-title-edit-wrapper .check-icon").attr("id", "create-note");
		$(".note-title-edit-text").html("Care este titlul notei?");
		$(".note-title-edit-input").focus().attr("placeholder", "Titlu...");
	});
			...
			success: function(data) {
				if(data == "done") {
					...
					setInterval(function() {
						i -= 1;
						$(".notification-wrapper:first .notification-description").text("Setările au fost resetate. Se reîmprospătează în " + i + " secunde.");
					}, 1000);
					$("body").css("pointer-events", "none");
					notify("Resetare Setări...", "Setările dvs. au fost resetate. Reîmprospătare în 5 secunde.", "purple", 4000);
					setTimeout(function() {
						location.reload();
					}, 5000);
				}
				else {
					notify("Eroare...", "Ceva nu a funcționat.", "red", 4000);
				}
			}
		});
	});
	...
	$(".settings-wrapper .apply-button").on("click", function() {
		...
		var config = $(".settings-wrapper .apply-button").attr("data-change");
		if(config == current) {
			notify("Eroare...", "Setările sunt deja aplicate în acest format.", "red", 4000);
		}
		else {
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					...
					setInterval(function() {
						i -= 1;
						$(".notification-wrapper:first .notification-description").text("Numele dvs. de utilizator a fost schimbat. Se reîmprospătează în " + i + " secunde.");
					}, 1000);
					$("body").css("pointer-events", "none");
					notify("Se schimbă numele de utilizator...", "Numele dvs. de utilizator a fost schimbat. Reîmprospătare în 5 secunde.", "purple", 10000);
					setTimeout(function() {
						$.ajax({
							type: "POST",
							data: { action: "logout" },
	...
	$(".settings-change-password-button").on("click", function() {
		...
		if(new_password == repeat_password) {
			$.ajax({
				...
				success: function(data) {
					if(data == "done") {
						...
						setInterval(function() {
							i -= 1;
							$(".notification-wrapper:first .notification-description").text("Parola dvs. a fost schimbată. Se reîmprospătează în " + i + " secunde.");
						}, 1000);
						$("body").css("pointer-events", "none");
						notify("Schimbare parolă...", "Parola dvs. a fost schimbată. Reîmprospătare în 5 secunde.", "purple", 5000);
						setTimeout(function() {
							$.ajax({
								type: "POST",
								data: { action: "logout" },
			...
			notify("Eroare...", "Parolele pe care le-ați introdus nu se potrivesc.", "red", 4000);
		}
	});
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					...
					close_settings_page();
					get_notes();
					notify("Ștergere Note...", "Notele dvs. au fost șterse.", "purple", 4000);
				}
				else {
					notify("Eroare...", data, "red", 4000);
				}
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					...
					set_global_settings();
					apply_settings();
					notify("Salvare Setări...", "Setările dvs. au fost salvate.", "purple", 4000);
					close_settings_page();
				}
				else {
					notify("Eroare...", "Ceva nu a funcționat.", "red", 4000);
				...
				success: function(data) {
					if(data != "empty") {
						...
						$.each(notes, function(key, value) {
							$(".settings-notes-pane").append('<div class="settings-note-wrapper"><span class="settings-note-wrapper-text settings-note-wrapper-id"><b>ID: </b><span class="spoiler">' + key + '</span></span><span class="settings-note-wrapper-text"><b>Titlu:</b> ' + value["title"] + '</span><span class="settings-note-wrapper-text settings-note-wrapper-locked"><b>Blocat:</b> ' + ucfirst(value["locked"].toString()) + '</span><span class="settings-note-wrapper-text settings-note-wrapper-shared"><b>Partajat:</b> ' + ucfirst(value["shared"].toString()) + '</span><span class="settings-note-wrapper-text settings-note-wrapper-date"><b>Ultima modificare:</b> ' + value["time_modified"] + '</span><button class="settings-note-delete-button" id="' + key + '.xnt">Ștergere</button></div>');
						});
					}
					else {
						$(".settings-notes-pane").show().html('<div class="settings-section" style="border:none;"><span class="settings-heading-text">Nu au fost găsite note.</span></div>');
					}
				}
			});
		}
		...
		$.ajax({
			...
			success: function(data) {
				if(data.length != 0) {
					...
					notify("Eroare...", "Preluarea preferințelor utilizatorului a eșuat.", "red", 4000);
					close_settings_pane();
				}
			}
		});
	...
	$(".note-lock-wrapper .check-icon").on("click", function() {
		...
		if(action == "lock-note") {
			...
			if(new_password == repeat_password) {
				...
				notify("Parolele nu se potrivesc", "Parolele pe care le-ați introdus nu se potrivesc.", "red", 4000);
	...
	$(".notes-list").delegate(".note-wrapper", "click", function() {
		...
		if(locked) {
			...
			$(".relock-button").show();
			if(!$(this).hasClass("active")) {
				open_note_lock("locked", "reveal-note", "Vizualizare Notă", "Acest lucru nu va elimina definitiv blocarea.");
	...
	function open_note_lock(state, action, title, description) {
		...
		if(action == "lock-note") {
			...
			$(".note-lock-wrapper .current-password").show().focus().attr("placeholder", "Parola...");
			...
			$(".note-lock-wrapper .current-password").show().focus().attr("placeholder", "Parola Curentă...");
	...
	function lock_note(file, password) {
		$.ajax({ 
			...
			success: function(data) {
				if(data == "done") {
					notify("blocare...", "Nota dvs. a fost blocată.", "blue", 4000);
					get_notes();
					setTimeout(function() {
						close_note();
					}, 250);
	...
	function unlock_note(file, password) {
		$.ajax({ 
			...
			success: function(data) {
				if(data == "done") {
					notify("Deblocare...", "Nota dvs. a fost deblocată.", "blue", 4000);
					get_notes();
					setTimeout(function() {
						close_note();
						setTimeout(function() {
					...
					notify("Eroare...", "Parola introdusă a fost incorectă.", "red", 4000);
				}
			}
		});
	}
	function relock_note(file, current_password, new_password) {
		$.ajax({ 
			...
			success: function(data) {
				if(data == "done") {
					notify("Reblocare...", "Parola notei dvs. a fost schimbată.", "blue", 4000);
					get_notes();
					close_note();
				}
				else {
	...
	function rename_note(file, title) {
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					notify("Redenumire...", "nota dvs. a fost redenumită.", "blue", 4000);
					get_notes();
					setTimeout(function() {
						adjust_note_facade();
					}, 250);
	...
	function create_note(title) {
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					notify("Creare...", "nota dvs. a fost creată.", "blue", 4000);
					get_notes();
					setTimeout(function() {
						if(global_setting_reopen_notes == "automatically") {
							$(".note-wrapper:first").trigger("click");
					...
					notify("Eroare...", "Ceva nu a functionat.", "red", 4000);
				}
			}
		});
	}
			...
			success: function(data) {
				if(locked) {
					if(data != "incorrect") {
						...
						$(".editor-content").html(data).attr({"placeholder":"Introduceți textul aici...", "id":"e-" + file});
					}
					else {
						close_note();
						notify("Eroare...", "Parola introdusă a fost incorectă.", "red", 4000);
	...
	function save_note(background) {
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					if(!background) {
						notify("Salcare...", "Nota dvs. a fost salvată.", "blue", 4000);
					}
					get_notes();
					setTimeout(function() {
						adjust_note_facade();
	...
	function delete_note(file, password, bypass) {
		$.ajax({
			...
			success: function(data) {
				if(data == "done") {
					notify("Ștergere...", "Nota dvs. a fost ștearsă.", "blue", 4000);
					close_note();
					get_notes();
				}
			}
		...
		$.ajax({
			...
			success: function(data) {
				if(data == "incorrect") {
					...
					notify("Accesare...", "Date brute extrase.", "blue", 4000);
	...
	function note_sharing(file, password, share) {
		if(share) {
			$.ajax({
				...
				success: function(data) {
					if(data == "done") {
						notify("Notă Publică...", "Nota dvs. este acum accesibilă public.", "green", 6500);
						get_notes();
						$(".public-button").hide();
						$(".private-button").show();
						$(".link-button").show();
			...
			$.ajax({
				...
				success: function(data) {
					if(data == "done") {
						notify("Notă Privată...", "Nota dvs. nu mai poate fi accesată public.", "green", 4000);
						get_notes();
						$(".public-button").show();
						$(".private-button").hide();
						$(".link-button").hide();
	...
	function close_note() {
		deselect_notes();
		$(".editor-content").addClass("editor-empty").attr({"contenteditable":"true", "placeholder":"Notă nouă...", "id":"", "data-title":"", "data-locked":"", "data-si":""}).data("pw", "").html("").blur();
	...
	function count_title_characters() {
		...
		if(remaining < -99) {
			...
			$(".note-title-edit-counter").html("Sigur?");
		}
		else {
	...
	function create_tooltips() {
		tippy('.compose-button', { content: "<span class='tooltip'>Notă Nouă</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.notes-button', { content: "<span class='tooltip'>Toate Notele</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.locked-button', { content: "<span class='tooltip'>Note Blocate</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.shared-button', { content: "<span class='tooltip'>Note Partajate</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.help-button', { content: "<span class='tooltip'>Ajutor</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.settings-button', { content: "<span class='tooltip'>Setări</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		tippy('.logout-button', { content: "<span class='tooltip'>Deconectare</span>", arrow: false, interactive: false, placement: "right", distance: "-50", duration: 200,  offset: "0"});
		...
		tippy('.action-save', { content: "<span class='tooltip'>Salvare</span>", arrow: false, interactive: false, placement: "bottom", distance: "5", duration: 200,  offset: "0"});
	}
	...
	function initialize() {
		...
		$(".editor-content").attr({"contenteditable":"true", "placeholder":"Notă Nouă...", "id":"", "data-title":"", "data-locked":"", "data-si":""}).data("pw", "");	
	}
